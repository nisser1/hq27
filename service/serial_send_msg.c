...
void *serial_send_msg(void *arg)
{
	...
	//printf("dd %d\n",ret);
	MSG msg;
	//unsigned short contrast = contrast_net;
	int i = 0;
	int cmd_own = 0;
	...
	*/	
	...
	while(thread_arg->thread_flag == 0)
	{
		...
		switch(cmd_own)
		{
			...
			case 0:
				//发送亮度命令		
				/*ret = serial_cmd_packet(WRITE_LIGHT,&msg,light,0);		
			...
			case 2:
				//写对比度命令	
				/*printf("write serial contrast_net=%d\n",contrast_net);
				ret = serial_cmd_packet(WRITE_CONTRAST,&msg,0,contrast_net);		
				if(serial_send(thread_arg->serial_fd, msg.data,msg.size) > 0)
				{
					printf("serial_send sucess\n");
				}
