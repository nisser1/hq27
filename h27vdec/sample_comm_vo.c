...
HI_S32 SAMPLE_COMM_VO_StartDev(VO_DEV VoDev, VO_PUB_ATTR_S* pstPubAttr)
{
    ...
    s32Ret = HI_MPI_VO_SetPubAttr(VoDev, pstPubAttr);		//设置视频输出设备的公共属性   使用高清设备0
    ...
    s32Ret = HI_MPI_VO_Enable(VoDev);						//启用视频输出设备
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_VO_StartLayer(VO_LAYER VoLayer, const VO_VIDEO_LAYER_ATTR_S* pstLayerAttr)
{
    ...
    s32Ret = HI_MPI_VO_SetVideoLayerAttr(VoLayer, pstLayerAttr);			//设置视频层属性
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    ...
    for (i = 0; i < u32WndNum; i++)
    {
        ...
        {
            stChnAttr.stRect.s32X       = ALIGN_DOWN((u32Width / u32Square) * (i % u32Square), 2);		//得到起点 坐标
            stChnAttr.stRect.s32Y       = ALIGN_DOWN((u32Height / u32Square) * (i / u32Square), 2);		
            stChnAttr.stRect.u32Width   = ALIGN_DOWN(u32Width / u32Square, 2);							//得到长和宽
		...
		if(enMode == VO_MODE_9CUS)
		{
			if(i==9)
			{
				stChnAttr.stRect.s32X       = ALIGN_DOWN(0, 2);								//得到起点 坐标
				stChnAttr.stRect.s32Y       = ALIGN_DOWN(0, 2);		
				stChnAttr.stRect.u32Width   = ALIGN_DOWN(1536, 2);							//得到长和宽
				stChnAttr.stRect.u32Height  = ALIGN_DOWN(864, 2);
				stChnAttr.u32Priority       = 0;
				stChnAttr.bDeflicker        = HI_FALSE;
			}
			...
			if( (i== 8) ||
				...
				(i== 5))
			{
				stChnAttr.stRect.s32X       = ALIGN_DOWN(1536, 2);							//得到起点 坐标
				stChnAttr.stRect.s32Y       = ALIGN_DOWN((8-i)*216, 2);		
				stChnAttr.stRect.u32Width   = ALIGN_DOWN(384, 2);							//得到长和宽
				stChnAttr.stRect.u32Height  = ALIGN_DOWN(216, 2);
				stChnAttr.u32Priority       = 0;
				stChnAttr.bDeflicker        = HI_FALSE;
			}
			if( (i==4 ) ||
				...
				(i== 0))
			{
				...
				stChnAttr.stRect.s32X       = ALIGN_DOWN(i*384, 2);					//得到起点 坐标
				stChnAttr.stRect.s32Y       = ALIGN_DOWN(864, 2);		
				stChnAttr.stRect.u32Width   = ALIGN_DOWN(384, 2);					//得到长和宽
				stChnAttr.stRect.u32Height  = ALIGN_DOWN(216, 2);
				stChnAttr.u32Priority       = 0;
				stChnAttr.bDeflicker        = HI_FALSE;		
		...
		if(enMode == VO_MODE_4CUS)
		{
			if( (i== 1) || (i== 0))
			{

				stChnAttr.stRect.s32X       = ALIGN_DOWN(0, 2);					//得到起点 坐标
				stChnAttr.stRect.s32Y       = ALIGN_DOWN(540 * i, 2);		
				stChnAttr.stRect.u32Width   = ALIGN_DOWN(960, 2);				//得到长和宽
				stChnAttr.stRect.u32Height  = ALIGN_DOWN(540, 2);
				stChnAttr.u32Priority       = 0;
				stChnAttr.bDeflicker        = HI_FALSE;		
			}
			if( (i== 3) || (i== 2))
			{
				...
				stChnAttr.stRect.s32X       = ALIGN_DOWN(960, 2);					//得到起点 坐标
				stChnAttr.stRect.s32Y       = ALIGN_DOWN(540 * (i-2), 2);		
				stChnAttr.stRect.u32Width   = ALIGN_DOWN(960, 2);					//得到长和宽
				stChnAttr.stRect.u32Height  = ALIGN_DOWN(540, 2);
				stChnAttr.u32Priority       = 0;
				stChnAttr.bDeflicker        = HI_FALSE;		
        ...
        s32Ret = HI_MPI_VO_SetChnAttr(VoLayer, i, &stChnAttr);	//设置指定视频通道的属性
        if (s32Ret != HI_SUCCESS)
        {
...
HI_S32 SAMPLE_COMM_VO_HdmiStart(VO_INTF_SYNC_E enIntfSync)
{
    ...
    HI_HDMI_ID_E        enHdmiId    = HI_HDMI_ID_0;					//HI_HDMI_ID_0 =0

    SAMPLE_COMM_VO_HdmiConvertSync(enIntfSync, &enVideoFmt);		//enVideoFmt = HI_HDMI_VIDEO_FMT_1080P_30
...
HI_S32 SAMPLE_COMM_VO_StartVO(SAMPLE_VO_CONFIG_S *pstVoConfig)//start VO1
{
    RECT_S                 stDefDispRect  = {0, 0, 1920, 1080};
    SIZE_S                 stDefImageSize = {1920, 1080};
