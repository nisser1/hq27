...
HI_S32 SAMPLE_COMM_VDEC_InitVBPool(HI_U32 ChnNum, SAMPLE_VDEC_ATTR *pastSampleVdec)
{
    ...
    VB_POOL_CONFIG_S stVbPoolCfg;	//视频缓存池结构体
    ...
    /* PicBuffer */
    for(j=0; j<VB_MAX_COMM_POOLS; j++)	   //VB_MAX_COMM_POOLS公共缓存池个数  16   更改所有公共缓存池的大小
    {
        bFindFlag = HI_FALSE;
        for(i=0; i<ChnNum; i++)
        {
            ...
            {
                stVbConf.astCommPool[j].u64BlkSize = astSampleVdecBuf[i].u32PicBufSize;	//缓存块大小
                stVbConf.astCommPool[j].u32BlkCnt  = pastSampleVdec[i].u32FrameBufCnt;	//缓存块个数
    ...
    /* TmvBuffer */
    for(j=pos+1; j<VB_MAX_COMM_POOLS; j++)		//VB_MAX_COMM_POOLS公共缓存池个数  16   更改所有公共缓存池的大小
    {
        bFindFlag = HI_FALSE;
        for(i=0; i<ChnNum; i++)
        {
            ...
            {
                stVbConf.astCommPool[j].u64BlkSize = astSampleVdecBuf[i].u32TmvBufSize;							//缓存块大小
                stVbConf.astCommPool[j].u32BlkCnt  = pastSampleVdec[i].stSapmleVdecVideo.u32RefFrameNum+1;      //缓存块个数
    ...
    if(VB_SOURCE_MODULE == g_enVdecVBSource)		//VB来源  模块VB
    {
...
HI_VOID * SAMPLE_COMM_VDEC_SendStream(HI_VOID *pArgs)
{
    ...
    prctl(PR_SET_NAME, "VideoSendStream", 0,0,0);//设置线程名字
	
	
#if 0	
	//20200814 syj change换成rtsp码流
    ...
    while (1)
    {
		...
		pthread_mutex_lock(&mutex);
		pu8Buf=&(pstVdecThreadParam->msg->recv_buf[pstVdecThreadParam->s32ChnId][pstVdecThreadParam->msg->rindex[pstVdecThreadParam->s32ChnId]]);
		pthread_mutex_unlock(&mutex);
#endif
        ...
        stStream.bEndOfFrame  = (pstVdecThreadParam->s32StreamMode==VIDEO_MODE_FRAME)? HI_TRUE: HI_FALSE; // 发送帧模式
...
SendAgain:
        ...
        {
			//发送失败  vdec解码未完成 等待重发 重发成功后再继续	
            usleep(pstVdecThreadParam->s32IntervalTime);
			printf("HI_MPI_VDEC_SendStream fail for 0x%x ,pstVdecThreadParam->s32ChnId =%d \n", s32Ret,pstVdecThreadParam->s32ChnId);
            goto SendAgain;
        }
        else
        {
...
HI_S32 SAMPLE_COMM_VDEC_Start(HI_S32 s32ChnNum, SAMPLE_VDEC_ATTR *pastSampleVdec)
{
    ...
    CHECK_RET(HI_MPI_VDEC_SetModParam(&stModParam), "HI_MPI_VDEC_GetModParam");		//设置解码参数
    ...
    for(i=0; i<s32ChnNum; i++)
    {
        ...
        CHECK_CHN_RET(HI_MPI_VDEC_CreateChn(i, &stChnAttr[i]), i, "HI_MPI_VDEC_CreateChn");			//创建解码通道
        ...
        CHECK_CHN_RET(HI_MPI_VDEC_GetChnParam(i, &stChnParam), i, "HI_MPI_VDEC_GetChnParam");		//获取解码通道参数
        if (PT_H264 == pastSampleVdec[i].enType || PT_H265 == pastSampleVdec[i].enType)
        {
            stChnParam.stVdecVideoParam.enDecMode         = pastSampleVdec[i].stSapmleVdecVideo.enDecMode;	
            stChnParam.stVdecVideoParam.enCompressMode    = COMPRESS_MODE_TILE;								//titlle压缩格式
            stChnParam.stVdecVideoParam.enVideoFormat     = VIDEO_FORMAT_TILE_64x16;						//titlle压缩格式  64*16
            if(VIDEO_DEC_MODE_IPB == stChnParam.stVdecVideoParam.enDecMode)									//解码3帧
            {
        ...
        CHECK_CHN_RET(HI_MPI_VDEC_SetChnParam(i, &stChnParam), i, "HI_MPI_VDEC_GetChnParam");			//设置解码通道

        CHECK_CHN_RET(HI_MPI_VDEC_StartRecvStream(i), i, "HI_MPI_VDEC_StartRecvStream");				//解码器开始接收数据
    }
