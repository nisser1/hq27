	...
	*/	
	while(thread_arg->thread_flag == 0)
	{
		...
		{		
			...
			analysis_net_message(buffer,thread_arg->serial_snedto_net);
			
			//serial_message.type = WRITE_CONTRAST;
			//serial_message.contrast_value = 10;
			
			if(sendto(thread_arg->serial_fd, thread_arg->serial_snedto_net, sizeof(SERIAL_DATA), 0, (struct sockaddr *)&(thread_arg->clientaddr),  (socklen_t)sizeof(testaddr)) < 0)
			{
